#################################################################
# 🐧 МАКЕТ БЛОЧНОЙ УСТАНОВКИ CACHY OS (BIOS + GPT + BTRFS + LVM + LUKS)
#################################################################
...
#################################################################
# 💾  [LIVE] БЛОК 5: РАЗМЕТКА ДИСКА (GPT + BIOS Boot) + LUKS + LVM
#################################################################
...
#    • sda2 — /boot (ext4), 1 ГБ, тип 8300 (НЕ шифруется!).
#    • sda3 — LUKS/LVM (остальное место), тип 8309.
...
sgdisk -n 2::+1G --typecode=2:8300 --change-name=2:'Boot' /dev/sdx
# 📦 РАЗДЕЛ 3: LUKS Crypt — остальное место, тип 8309
# Назначение: LUKS шифрование + LVM + BTRFS.
sgdisk -n 3::-0 --typecode=3:8309 --change-name=3:'LUKS Crypt' /dev/sdx
#------------------------------------------------------------------------------
...
# • Swap находится внутри LVM, отдельного раздела swap нет.
...
# /boot раздел (ext4)        # sda2
# LUKS/LVM раздел            # sda3
#############################################################
#
#################################################################
...
#################################################################
# ℹ️ Зачем: Форматирование /boot, создание LUKS-контейнера,
...
# ℹ️ ПЕРЕД ВЫПОЛНЕНИЕМ: Убедитесь, что 'sda1', 'sda2', 'sda3'
...
#    • sda2 — /boot (ext4, НЕ шифруется!).
#    • sda3 — LUKS/LVM (Btrfs на lv_root, swap на lv_swap).
#################################################################
# 🧹 Очистка экрана терминала
clear
#------------------------------------------------------------------------------
# ШАГ 1: ФОРМАТИРОВАНИЕ /BOOT (НЕ ШИФРУЕТСЯ)
#------------------------------------------------------------------------------
# 📦 Форматирование /boot раздела в ext4.
# /boot должен быть доступен GRUB до ввода пароля, поэтому он не шифруется.
...
# 📦 Монтирование /boot раздела (ext4).
...
# ✅ ИСПРАВЛЕНО: Универсальная команда с экранированными скобками.
...
#    для работы с LUKS, LVM, Btrfs и гибернацией.
# 💡 Включает: GRUB, grub-btrfs, plymouth, LUKS, LVM, resume.
# ❗ КРИТИЧЕСКИ ВАЖНО: Без корректных настроек система НЕ ЗАГРУЗИТСЯ!
# 📌 BIOS Boot Partition (sda1) не форматируется и не монтируется.
# 📌 /boot находится на отдельном незашифрованном разделе (ext4).
#################################################################
# 🧹 Очистка экрана терминала
clear
pacman -Syy
...
# ✅ ИСПРАВЛЕНО: Используется (.*) для корректного захвата строки.
...
# ✅ ИСПРАВЛЕНО: Универсальная команда с (.*).
# Работает независимо от того, пустой MODULES или нет.
sed -i 's/^MODULES=(\(.*\))/MODULES=(\1 btrfs dm_mod dm_crypt)/' /etc/mkinitcpio.conf
#------------------------------------------------------------------------------
...
# ✅ ИСПРАВЛЕНО: Используется .* для замены всей строки.
# Внутренние кавычки экранированы как ".
...
#------------------------------------------------------------------------------
# ШАГ 5: ГЕНЕРАЦИЯ КОНФИГУРАЦИИ И ПЕРЕСБОРКА
#------------------------------------------------------------------------------
...
echo " "
echo "#####################################################"
echo "## ✅ ЗАГРУЗЧИК И ЯДРО НАСТРОЕНЫ (BIOS+LUKS+LVM)  ##"
echo "#####################################################"
echo " "
#################################################################
