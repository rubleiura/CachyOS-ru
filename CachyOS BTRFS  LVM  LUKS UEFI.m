...
# • Имена устройств (cryptlvm, vg_main, lv_cachyos) можно оставить как есть.
# 💡 СОВЕТ ПО РЕДАКТИРОВАНИЮ:
# • Если в Live-среде доступен графический редактор, используйте его для замены.
# • Иначе используйте поиск и замену в текстовом редакторе или команду sed.
# 💡 ПРОВЕРКА:
...
# • sda1 — ESP (FAT32, /boot).
# • sda2 — LUKS/LVM (Btrfs на lv_cachyos, swap на lv_swap).
# #################################################################
# 🧹 Очистка экрана терминала
clear
# #------------------------------------------------------------------------------
...
# 📦 Создание логического тома для cahyos (всё оставшееся место).
lvcreate -l 100%FREE vg_main -n lv_cachyos
# #------------------------------------------------------------------------------
# ШАГ 4: ФОРМАТИРОВАНИЕ BTRFS И ПОДТОМЫ
# #------------------------------------------------------------------------------
# 📦 Форматирование cahyos тома в BTRFS.
mkfs.btrfs -f /dev/vg_main/lv_cachyos
# 📦 Монтирование cahyos тома временно для создания подтомов.
mount /dev/vg_main/lv_cachyos /mnt
...
# @cache — каталог кэша приложений и пакетного менеджера
btrfs subvolume create /mnt/@cache
# @tmp — каталог для временных файлов
btrfs subvolume create /mnt/@tmp
# @log — каталог системных и прикладных журналов (логов)
...
mount -o defaults,subvol=@ /dev/vg_main/lv_cachyos /mnt
# 📦 Создание необходимых директорий.
mkdir -p /mnt/{/boot,root,home,srv,var/log,var/tmp,var/cache,var/lib/machines,var/lib/portables}
# 📦 Монтирование остальных подтомов.
mount -o defaults,subvol=@root /dev/vg_main/lv_cachyos /mnt/root
mount -o defaults,subvol=@home /dev/vg_main/lv_cachyos /mnt/home
mount -o defaults,subvol=@srv /dev/vg_main/lv_cachyos /mnt/srv
mount -o defaults,subvol=@log /dev/vg_main/lv_cachyos /mnt/var/log
mount -o defaults,subvol=@tmp /dev/vg_main/lv_cachyos /mnt/var/tmp
mount -o defaults,subvol=@cache /dev/vg_main/lv_cachyos /mnt/var/cache
...
sed -i "s|^HOOKS=(.*)|HOOKS=(base udev autodetect modconf kms block keyboard keymap encrypt lvm2 resume filesystems fsck)|" /etc/mkinitcpio.conf
...
# 📋 Получение UUID cahyos раздела
ROOT_UUID=$(blkid -s UUID -o value /dev/vg_main/lv_cachyos)
# 📋 Настройка GRUB_CMDLINE_LINUX_DEFAULT
# ✅ Замена всей строки с параметрами загрузки.
sed -i "s|^GRUB_CMDLINE_LINUX_DEFAULT=.*|GRUB_CMDLINE_LINUX_DEFAULT=\"udev.log_priority=3 quiet splash resume=UUID=${SWAP_UUID}\"|" /etc/default/grub
# 📋 Настройка GRUB_CMDLINE_LINUX
# ✅ Полная замена строки с параметрами ядра.
# ✅ Внутренние кавычки экранированы для безопасности.
sed -i "s|^GRUB_CMDLINE_LINUX=.*|GRUB_CMDLINE_LINUX=\"cryptdevice=UUID=${CRYPT_UUID}:cryptlvm root=UUID=${ROOT_UUID}\"|" /etc/default/grub
...
clear
 #------------------------------------------------------------------------------
# # 1. 🛡️ СИСТЕМНОЕ ЯДРО, КЛЮЧИ И БАЗОВЫЕ ЗАВИСИМОСТИ
# #------------------------------------------------------------------------------
...
pacman -Sy --noconfirm cachyos-hooks cachyos-settings device-mapper efitools logrotate rebuild-detector perl poppler-glib
#------------------------------------------------------------------------------
# # 2.  🌐 СЕТЬ, ЗЕРКАЛА И БЕЗОПАСНОСТЬ
# #------------------------------------------------------------------------------
...
pacman -Sy --noconfirm cachyos-rate-mirrors networkmanager-openvpn iwd wireless-regdb nss-mdns ethtool dnsmasq modemmanager xl2tpd bind ufw ufw-extras
# #------------------------------------------------------------------------------
# # 3. 🔧 ОБОРУДОВАНИЕ, BLUETOOTH И ДИАГНОСТИКА
# #------------------------------------------------------------------------------
...
# #------------------------------------------------------------------------------
# # 4. 🖥️ СИСТЕМНЫЕ ДЕМОНЫ, БИБЛИОТЕКИ И НАСТРОЙКИ СРЕДЫ
# #------------------------------------------------------------------------------
...
pacman -Sy --noconfirm accountsservice plocate libgsf libopenraw cachyos-hello
# #------------------------------------------------------------------------------
# # 5. 📦 УПРАВЛЕНИЕ ПАКЕТАМИ, CLI-УТИЛИТЫ И РАЗРАБОТКА
# #------------------------------------------------------------------------------
...
pacman -Sy --noconfirm paru pacman-contrib pkgfile cachy-update glances duf meld lsb-release cachyos-packageinstaller shelly
# #------------------------------------------------------------------------------
# # 6. 💻 РЕДАКТОР
# #------------------------------------------------------------------------------
# [✅] 📦 nano | 📝 Простой консольный редактор. Резервный вариант.
# [✅] 📦 nano-syntax-highlighting | 🌈 Дополнительные схемы подсветки для nano.
pacman -Sy --noconfirm nano nano-syntax-highlighting
# #------------------------------------------------------------------------------
# # 🔋 ПИТАНИЕ, АРХИВЫ И УТИЛИТЫ ФАЙЛОВЫХ СИСТЕМ
# #------------------------------------------------------------------------------
...
pacman -Sy --noconfirm power-profiles-daemon upower cpupower nilfs-utils fsarchiver dmraid mtools
# #------------------------------------------------------------------------------
# # 7. 🔋 КОЛЛЕКЦИЯ ОФИЦИАЛЬНЫХ ОБОЕВ CACHY OS
# #------------------------------------------------------------------------------
# ⚠️ Коллекция официальных обоев CachyOS
# 📦 cachyos-wallpapers
pacman -Sy --noconfirm cachyos-wallpapers
# #------------------------------------------------------------------------------
# # 8. ℹ️ БАЗОВЫЕ УТИЛИТЫ И АРХИВАТОРЫ
# #------------------------------------------------------------------------------
...
pacman -Sy --noconfirm haveged wget usbutils lsof dialog zip unzip unrar p7zip lzop lrzip sudo less bash-completion
# #------------------------------------------------------------------------------
# # 9. ℹ️ УТИЛИТЫ АДМИНИСТРИРОВАНИЯ И МОНИТОРИНГА
# #------------------------------------------------------------------------------
...
# #------------------------------------------------------------------------------
# # 10. ℹ️ ПОДДЕРЖКА ФАЙЛОВЫХ СИСТЕМ И СЕТЕЙ
# #------------------------------------------------------------------------------
...
# #------------------------------------------------------------------------------
# # 13. ℹ️ XDG, МОНТИРОВАНИЕ И АУДИО
# #------------------------------------------------------------------------------
...
pacman -Sy --noconfirm pipewire-alsa pipewire-pulse pipewire-jack pipewire-v4l2 pipewire-zeroconf
# 📦 wireplumber — менеджер сессий PipeWire.
pacman -Sy --noconfirm wireplumber
systemctl --global enable pipewire pipewire-pulse wireplumber
# #------------------------------------------------------------------------------
# # 14. ℹ️ АУДИО, ВИДЕО И КОДЕКИ
# #------------------------------------------------------------------------------
...
pacman -Sy --noconfirm alsa-utils alsa-plugins alsa-firmware pavucontrol gst-plugin-pipewire
...
# #------------------------------------------------------------------------------
# # 15. ℹ️ ДОКУМЕНТАЦИЯ И СЕТЕВЫЕ УТИЛИТЫ
# #------------------------------------------------------------------------------
...
fc-cache -fv
# #------------------------------------------------------------------------------
#  # 17. ℹ️ ЗАГРУЗЧИК, BTRFS И СНЕПШОТЫ
# #------------------------------------------------------------------------------
...
# #------------------------------------------------------------------------------
# # 18. ℹ️ АКТИВАЦИЯ СЛУЖБ И АВТО-НАСТРОЙКА ДРАЙВЕРОВ (CHROOT)
# #------------------------------------------------------------------------------
...
clear
# #------------------------------------------------------------------------------
# # 19. ℹ️ ДРАЙВЕРЫ ВИДЕОКАРТЫ (АВТОМАТИЧЕСКАЯ УСТАНОВКА)
# #------------------------------------------------------------------------------
...
pacman -Sy --noconfirm plasma kde-system-meta dolphin-plugins kate konsole gwenview elisa okular ark cachyos-kde-settings
# pacman -Sy --noconfirm skanpage skanlite
