...
# ВАРИАНТ I (При установке с помощью Live Arch Linux)
...
# ВАРИАНТ II (После установки Cachy OS)

# ========================================================================
# ПОЛНЫЙ ЧЕК-ЛИСТ НАСТРОЙКИ ZSH + AGNOSTER В CACHYOS
# Просто выделите весь этот блок, скопируйте и вставьте в терминал.
# Все строки, начинающиеся с #, будут проигнорированы оболочкой.
# ========================================================================

# ШАГ 0: Переключение оболочки по умолчанию с Fish на Zsh
# В CachyOS по умолчанию используется Fish, нам нужен Zsh
...
# ВАЖНО: После выполнения этой команды нужно либо ПЕРЕЗАПУСТИТЬ терминал,
# либо выполнить команду ниже для немедленного переключения:
exec zsh

# ШАГ 1: Установка Zinit
# Устанавливаем zinit-git через paru (стандартный помощник AUR в CachyOS)
paru -S zinit-git --noconfirm

# Находим точный путь к файлу zinit.zsh и сохраняем в переменную
ZINIT_PATH=$(pacman -Ql zinit-git | grep 'zinit.zsh$' | awk '{print $2}')

# Выводим подтверждение
echo "✅ Путь к zinit найден: $ZINIT_PATH"

# ШАГ 2: Создание чистого конфигурационного файла ~/.zshrc
# Записываем инициализацию zinit в первую строку
echo "source $ZINIT_PATH" > ~/.zshrc

# Добавляем рабочий блок настроек (без комментариев внутри конфига)
cat >> ~/.zshrc << 'EOF'
...
zinit ice atinit"autoload -Uz promptinit && promptinit"
...
echo "✅ Файл ~/.zshrc успешно создан и заполнен."

# ШАГ 3: Применение настроек и очистка кэша
# Очищаем кэш zinit от предыдущих неудачных попыток
rm -rf ~/.zinit/snippets/OMZ* 2>/dev/null

# Перезагружаем конфигурацию
source ~/.zshrc

echo "🎉 Настройка завершена!"
echo "⚠️ ВАЖНО: Если вместо стрелочек вы видите квадратики, зайдите в настройки"
echo "вашего терминала и смените шрифт на любой Nerd Font (например, MesloLGS NF)."
